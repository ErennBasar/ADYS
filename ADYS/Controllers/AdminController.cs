...
namespace ADYS.Controllers
{
    ...
    public class AdminController : Controller
    {
        ...
        public async Task<ActionResult> Dashboard()
        {
            ...
            List<StudentDashboardDto> studentsWithAdvisor = new List<StudentDashboardDto>();
            ...
            using (var client = new HttpClient())
            {
                ...
                var response = await client.GetAsync($"api/AdminApi/Dashboard");

                if (response.IsSuccessStatusCode)
                {
                    studentsWithAdvisor = await response.Content.ReadAsAsync<List<StudentDashboardDto>>();
                }
                else
                {
                    TempData["ErrorMessage"] = "API'den veriler çekilemedi.";
                }
                return View(studentsWithAdvisor);
            }
        }
        ...
        public async Task<ActionResult> ManageStudents()
        {
            ...
            List<StudentDashboardDto> studentsWithAdvisor = new List<StudentDashboardDto>();
            List<SelectListItem> advisorSelectList = new List<SelectListItem>();
            ...
            using (var client = new HttpClient())
            {
                ...
                // Öğrenci-danışman eşleştirmesi
                var response1 = await client.GetAsync("api/AdminApi/ManageStudents");
                if (response1.IsSuccessStatusCode)
                {
                    studentsWithAdvisor = await response1.Content.ReadAsAsync<List<StudentDashboardDto>>();
                }

                // Danışman listesi
                var response2 = await client.GetAsync("api/AdminApi/GetAdvisors");
                if (response2.IsSuccessStatusCode)
                {
                    var advisors = await response2.Content.ReadAsAsync<List<dynamic>>();
                    advisorSelectList = advisors
            ...
            ViewBag.Advisors = new SelectList(advisorSelectList, "Value", "Text");
            return View(studentsWithAdvisor);
        }
        ...
        //        }
        public async Task<ActionResult> ManageAdvisors()
        {
            if (Session["UserRole"] == null || Session["UserRole"].ToString() != "Admin")
            {
                TempData["ErrorMessage"] = "Bu sayfaya erişmek için giriş yapmalısınız.";
            ...
            List<AdvisorDto> advisors = new List<AdvisorDto>();
            ...
            using (var client = new HttpClient())
            {
                ...
                var response = await client.GetAsync("api/AdminApi/ManageAdvisors");

                if (response.IsSuccessStatusCode)
                {
                    advisors = await response.Content.ReadAsAsync<List<AdvisorDto>>();
                }
                else
                {
                    TempData["ErrorMessage"] = "Danışman listesi API'den alınamadı.";
                }
        ...
        public async Task<ActionResult> ManageCourses()
        {
            ...
            //ManageCoursesResponseDto apiResult = null;
            List<CourseDto> courses = new List<CourseDto>();
            ManageCoursesResponseDto apiResult = null;

            using (var client = new HttpClient())
            {
                client.BaseAddress = new Uri("https://localhost:44335/"); // API adresin
                client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                var response =await client.GetAsync("api/AdminApi/ManageCourses");                                  

                if (response.IsSuccessStatusCode)
                {
                    apiResult = await response.Content.ReadAsAsync<ManageCoursesResponseDto>();

                    courses = apiResult.Courses;

                    // Ders listesini view'a gönder
                    //return View(apiResult.Courses); // View model olarak List<CourseDto> beklemeli
                }
                else
                {
                    TempData["ErrorMessage"] = "API'den veri alınamadı.";
                    return View(new List<CourseDto>()); // boş liste döndür
                }
            }
                    
            ViewBag.Advisors = new SelectList(apiResult.Advisors, "UserId", "UserName");

            ViewBag.Departments = new SelectList(apiResult.Departments, "DepartmentId", "DepartmentName");
        ...
        public async Task<ActionResult> ManageDepartments()
        {
            ...
            using (HttpClient client = new HttpClient())
            {
                ...
                var response = await client.GetAsync("https://localhost:44335/api/AdminApi/GetDepartments");

                if (response.IsSuccessStatusCode)
                {
                    var jsonData = await response.Content.ReadAsStringAsync();
                    var departments = JsonConvert.DeserializeObject<List<DepartmentDto>>(jsonData);
        ...
        public async Task<ActionResult> Review(int studentId)
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                var response = await client.GetAsync($"api/AdminApi/Review?studentId={studentId}");
                if (response.IsSuccessStatusCode)
                {
                    var result = await response.Content.ReadAsAsync<dynamic>();

                    ViewBag.StudentName = result.StudentName;
                    ViewBag.StudentId = result.StudentId;

                    var courseList = Newtonsoft.Json.JsonConvert.DeserializeObject<List<ReviewCourseDto>>(result.Courses.ToString());
                    return View(courseList);
                }
                else
                {
                    TempData["ErrorMessage"] = "API'den veriler alınamadı.";
        ...
        //            return View(terms);
        //        }
        public async Task<ActionResult> ManageTerms()
        {
            if (Session["UserRole"] == null || Session["UserRole"].ToString() != "Admin")
            {
                TempData["ErrorMessage"] = "Bu sayfaya erişmek için giriş yapmalısınız.";
            ...
            List<TermsDto> terms = new List<TermsDto>();
            using (var client = new HttpClient())
            {
                var response = await client.GetAsync($"https://localhost:44335/api/AdminApi/ManageTerms");

                if (response.IsSuccessStatusCode)
                {
                    terms = await response.Content.ReadAsAsync<List<TermsDto>>();
                }
                else
                {
                    TempData["ErrorMessage"] = "Dönemler API'den getirilemedi.";
                    return View(new List<TermsDto>());
                }
            }
